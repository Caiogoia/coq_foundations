...
Definition member (v : nat) (s : bag) : bool :=
  if (0 <? (count v s)) then true else false.
...
Fixpoint subset (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | nil => true
  | h :: t => if ((count h s1) <=? (count h s2)) then subset t s2 else false
  end.  
...
Proof.
  ...
  - rewrite F. rewrite plus_1_l. reflexivity.
Qed.
...
Proof.
  intros s1 s2 v. induction s1 as [| h s1' IHs1'].
  - simpl. reflexivity.
  - simpl.
    destruct (h=?v) eqn:E.
...
(*Proof.
  ...
  - simpl. induction l2 as [| h2 l2' IHl2'].
    -- simpl. reflexivity.
    -- assert (H:(rev(h2 :: l2') = h2 :: l2')). {
...
Proof.
  ...
  - simpl. rewrite eqb_top. reflexivity. 
Qed.

Theorem eqb_id_top : forall x, eqb_id x x = true.
Proof.
  intros x. destruct x eqn:E.
  - simpl. rewrite eqb_top. reflexivity. 
Qed.
...
Proof.
  intros d x v. simpl. rewrite eqb_id_top. reflexivity.
Qed.
(** [] *)
...
(* The type [baz] has only 1 _element_, because there is only one way to define it, with a series of boolean types.*)
