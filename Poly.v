...
Definition filter_even_gt7 (l : list nat) : list nat :=
  match l with
  | nil => nil
  | l1 => filter (fun n => negb (oddb n || (n <? 7))) l1
  end.
...
match l with
  | nil => (nil, nil)
  | t => (filter test t, filter (fun n => negb (test n)) t)
end.
...
Theorem aux : forall (X Y : Type) (f : X -> Y) (h : X),
  [f h] = map f [h].
Proof.
  intros X Y f h.
  simpl. reflexivity.
...
Theorem aux1 : forall (X Y : Type) (f : X -> Y) (l1 l2 : list X),
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. assert(H : rev (map f l') = map f (rev l')). {
      rewrite IHl'. reflexivity.
    }
    rewrite H. rewrite aux. rewrite aux1. simpl. reflexivity.
Qed.    
(** [] *)
...
(* Fold add where we can construct a list with the given elements.*)
...
Proof.                                   
  ...
  - simpl. reflexivity.
  - simpl.
    assert (H : fold (fun (a : X) (b : list Y) => f a :: b) l' [ ] = fold_map f l'). {
      simpl. destruct l' eqn:E.
      - simpl. reflexivity.
      - simpl. reflexivity. 
    }
...
*)
(* By induction in n:
   n = 0, length l = 0 -> @nth_error X l 0 = None, (thats true)
   
   IHn': forall X l n', length l = n' -> @nth_error X l n' = None (hypothesis)
   
   n = S n', length = S n' -> @nth_error X l (S n') = None
   
   Suppose l = h :: t,
   length l = S n' -> @nth_error X t n' = None
   but length t = n', so by the hypothesis this is true
   Qed.
...
Definition succ (n : cnat) : cnat := fun (X : Type) (f : X -> X) (x : X) => n X f (f x).
